...
from account_serializer import serialize_account
...
class ClientChannel:
    ...
    def encrypt_packet(self, clear_pkt: bytes) -> bytes:
        """prefix(4) клиром; всё после префикса шифруется с padding+MAGIC+wastage."""
        if self.bf_key is None:
            return clear_pkt
        PREFIX, BLOCK, FOOTER = 4, 8, 5
        pre, clear = clear_pkt[:PREFIX], clear_pkt[PREFIX:]
        pad = (BLOCK - ((len(clear) + FOOTER) % BLOCK)) % BLOCK
        data = clear + b'\x00' * pad + self._ENC_MAGIC + bytes([pad + 1])
        enc = BlowfishLesta(self.bf_key).encrypt(data)   # C[i]=ECB(P[i]^P[i-1]^C[i-1]); prev=0
        return pre + enc

    def decrypt_packet(self, enc_pkt: bytes) -> bytes:
        """Обратная операция: снять MAGIC+wastage, вернуть clear prefix+body."""
        if self.bf_key is None:
            return enc_pkt
        PREFIX = 4
        pre, body = enc_pkt[:PREFIX], enc_pkt[PREFIX:]
        dec = BlowfishLesta(self.bf_key).decrypt(body)
        w = dec[-1]
        if dec[-5:-1] != self._ENC_MAGIC:
            raise ValueError('bad enc magic: ' + dec[-5:].hex())
        return pre + dec[:len(dec) - w - 4]
...
def _parse_chan_footers(data: bytes):
    """Разбор on-channel пакета клиента (data ВКЛЮЧАЕТ 4-байтовый prefix).
    Парсим футеры С КОНЦА по флагам (порядок wire: msg-end ->
    [last_reliable?][seq?][acks?][cumulative_ack?][checksum?]).
    Возвращает (flags, seq, cumulative_ack). Подтверждено на захватах
    0x1448 / 0x144C / 0x045A."""
    flags = struct.unpack_from('<H', data, 4)[0]   # после prefix(4)
    pos = len(data)
    if flags & 0x0100:                # HAS_CHECKSUM
        pos -= 4
    cum_ack = None
    if flags & 0x0400:                # HAS_CUMULATIVE_ACK (последний перед checksum)
        pos -= 4; cum_ack = struct.unpack_from('<I', data, pos)[0]
    if flags & 0x0004:                # HAS_ACKS: [ack_seq*count][ack_count:1]
        ack_count = data[pos-1]; pos -= 1 + 4 * ack_count
    seq = None
    if flags & 0x0040:                # HAS_SEQUENCE_NUMBER
        pos -= 4; seq = struct.unpack_from('<I', data, pos)[0]
    return flags, seq, cum_ack


def _client_seq_from_packet(data: bytes) -> int:
    """Совместимость: вернуть seq клиента (корректно по флагам)."""
    _, seq, _ = _parse_chan_footers(data)
...
def _xor_cs(data: bytes) -> int:
    """XOR checksum строго как Packet::writeChecksum (packet.cpp).
    Движок XOR-ит 4-байтовые слова от начала до позиции checksum; последнее
    слово залезает в обнулённое поле checksum, т.е. ВКЛЮЧАЕТ хвост 1..3 байта,
    если длина тела не кратна 4. Раньше хвост терялся → on-channel reply с
    ненулевым хвостом (bootstrap/seq>0xFFFFFF) падал по checksum.
    Для тел кратных 4 (LoginApp, 28B) поведение идентично прежнему."""
    padded = data + b'\x00\x00\x00\x00'   # обнулённое поле checksum для overrun
    cs = 0
    off = 0
    n = len(data)
    while off < n:
        cs ^= struct.unpack_from('<I', padded, off)[0]
        off += 4
...
class BaseAppStub:
    ...
    def _send_on_channel(self, addr, ch, element: bytes):
        ...
        Открытый пакет: prefix(4, клир) + [ flags(2) + element + seq(4) + checksum(4) ]
        flags = ON_CHANNEL|IS_RELIABLE|HAS_SEQUENCE_NUMBER|HAS_CHECKSUM = 0x0158.
        Внешний reliable reply по preparePackets(): футеры только [seq][checksum].
        НЕТ CREATE_CHANNEL/version/id (это только indexed internal-каналы).
        encrypt_packet оставляет prefix(4) клиром, шифрует всё после него."""
        # Внешний канал всегда reliable -> seq нумеруется каналом; первый = 0.
        if not getattr(ch, 'established', False):
            seq = 0
            ch.our_seq = 1
        else:
            seq = ch.next_seq()
        # ON_CHANNEL(0x0008)|IS_RELIABLE(0x0010)|HAS_SEQUENCE_NUMBER(0x0040)|HAS_CHECKSUM(0x0100)
        flags  = 0x0008 | 0x0010 | 0x0040 | (0x0100 if BASEAPP_USE_CHECKSUM else 0)
        offset = getattr(ch, 'prefix_offset', 0)

        body = bytearray()
        body += struct.pack('<H', flags)         # flags
        body += bytes(element)                    # message element(s)
        body += struct.pack('<I', seq)           # sequence_num footer [HAS_SEQUENCE_NUMBER]
        if BASEAPP_USE_CHECKSUM:
            cs = _xor_cs(bytes(body))
            body += struct.pack('<I', cs)        # XOR checksum footer (последним)

        prefix    = _prefix_hash(bytes(body), offset)
        clear_pkt = struct.pack('<I', prefix) + bytes(body)
        pkt       = ch.encrypt_packet(clear_pkt)   # prefix клиром, остальное шифр

        self.sock.sendto(pkt, addr)
        self._log(
            f'>> on-channel seq={seq} flags=0x{flags:04X} {len(pkt)}B '
            f'bf={"ON" if ch.bf_key else "OFF"} (no CREATE_CHANNEL) hex={pkt[:24].hex(" ")}',
            addr)
        return pkt
    ...
    def _send_player_bootstrap(self, addr, ch):
        ...
        # 1) СОЗДАЕМ ДАННЫЕ АККАУНТА (Полный список свойств из Account.def)
        account_data = {
            "name": self.account_name,
            "normalizedName": self.account_name.upper(),
            "globalRating": 10000,
            "ver": 1,
            "accountType": 1,
            "attrs": 0,
            ...
            "peripheryID": 0,
            "saveTime": 0,
            "lastPlayerActivityTime": 0,
            "vehDossiersCutVer": 0,
            "vehDossiersVer": 0,
            "nextOffloadToPeripheryTime": 0,
            "walletID": 0,
            "extWalletID": 0,
            "pdata": "",
            
            # PYTHON-свойства ОБЯЗАТЕЛЬНО дампим в байты через протокол 2 
            # (BigWorld поддерживает только protocol=2)
            "bp": pickle.dumps(None, protocol=2),
            "initialServerSettings": pickle.dumps({
                "isB2P": False,
                "roaming": (0, "RU", "RU"),
                "wgcg": {},
                "premiumPlusBonus": {},
                
                # Добавленные ключи для предотвращения KeyError на клиенте
                "file_server": {
                    "upload_url": "",
                    "download_url": "",
                },
                "disabledPersonalMissions": False,
                "gui_settings": {},
        ...
        # 2) СЕРИАЛИЗУЕМ (МАГИЯ СОРТИРОВКИ И УПАКОВКИ BIGWORLD)
        # Убедись, что импортировал serialize_account в начале server_stub.py!
        packed_properties = serialize_account(account_data)

        # 3) createBasePlayer(Account)
        el = build_account_create_base_player_element(
            entity_id=self.account_entity_id,
            properties_data=packed_properties  # Передаем байты напрямую!
        )
        self._send_on_channel(addr, ch, el)

        # 4) receiveProperties (stats/inventory/serverSettings)
    ...
    def _dispatch(self, data: bytes, addr):
        ...
        ch.prefix_offset = 0  # принятый LoginSuccess валидируется клиентом с offset=0
        # ── On-channel входящие зашифрованы ЦЕЛИКОМ после префикса (RUN21) ──
        # Канал вооружён (bf_key) → пробуем расшифровать; валидный DEADBEEF = on-channel.
        if ch.bf_key is not None and len(data) > 8 and (len(data) - 4) % 8 == 0:
            try:
                _dec = ch.decrypt_packet(data)
                data = _dec
                self._log(f'   [ON-CHANNEL DECRYPTED] {len(data)}B '
                          f'flags=0x{struct.unpack_from("<H", data, 4)[0]:04X} '
                          f'elt_id=0x{(data[6] if len(data) > 6 else 0):02X} '
                          f'hex={data[:20].hex(" ")}', addr)
            except Exception:
                pass   # off-channel plaintext (ретрансмит ClientAuth) — оставить сырым
        ...
        if (flags & FLAG_HAS_REQUESTS) and len(data) >= 14:
            ...
            if elt_id in (0x00, 0x02):  # LoginKey
                ...
                self._log(f'   raw ClientAuth hex={data.hex(" ")}', addr)  # X-RAY #19: найти offset EF BE AD DE
                ...
                # ── ШАГ 1 (RUN24): ServerSessionKey OFF-CHANNEL PLAINTEXT ──────────
                # ДОКАЗАНО: клиент шлёт ClientAuth открытым текстом (flags=0x0001, без
                # ON_CHANNEL) ⇒ его EncryptionFilter ещё НЕ включён ⇒ on-channel ответ
                # расшифровать не может (дроп→ретрансмит). LoginApp reply (off-channel,
                # тот же bf_key/режим BlowfishLesta) клиент ПРИНИМАЕТ. Значит и
                # ServerSessionKey шлём OFF-CHANNEL plaintext; шифрование канала
                # включаем уже для bootstrap (шаг 3, после эха ClientSessionKey).
                try:
                    # ── ШАГ 1 (FIX): ServerSessionKey ON-CHANNEL ENCRYPTED ──────
                    # authenticate() в клиенте требует header.pChannel != NULL =>
                    # reply ОБЯЗАН прийти on-channel зашифрованным (proxy.cpp:
                    # setEncryption ДО send). Канал ключуется login blob (bf_key).
                    sk_payload = struct.pack('<I', session_key)        # ServerSessionKey: 4B
                    elem = build_reply_element(request_id, sk_payload)
                    ch.setup_blowfish(session_key)   # ВКЛЮЧИТЬ канал ПЕРВЫМ (bf_key = BASEAPP_BF_KEY blob)
                    pkt = self._send_on_channel(addr, ch, elem)  # prefix + flags 0x0158 + [seq][cs], шифр
                    self._log(f'   >> ServerSessionKey ON-CHANNEL key=0x{session_key:08X} req=0x{request_id:08X} ({len(pkt)}B)', addr)
                except Exception as e:
                    import traceback as _tb
                    self._log(f'   ServerSessionKey send error: {e} | {_tb.format_exc()}', addr)
                return

            elif elt_id == 0x01:  # ClientSessionKey (ШАГ 2 — эхо ServerSessionKey)
                self._log('<< Client SessionKey confirmation (ШАГ 2)', addr)
                ch.established = True
                if not getattr(ch, 'bootstrapped', False):
                    ch.bootstrapped = True
                    try:
                        self._send_player_bootstrap(addr, ch)
                        self._log('>> player bootstrap sent (ШАГ 2)', addr)
                    except Exception as e:
                        import traceback as _tb
        ...
        if (flags & FLAG_IS_ON_CHANNEL):
            ...
            # seq клиента читаем С НАЧАЛА (после сообщения), НЕ с конца:
            # растущие пакеты несут PIGGYBACKS (0x02) -> конец = piggyback, не футер.
            # SessionKey echo (elt_id=0x01) = Fixed(4): msg = id(1)+key(4)=5B,
            # сразу за ним seq(4) @ offset prefix4+flags2+5 = 11.
            cseq = None
            if msg_id == 0x01 and len(data) >= 15:
                cseq = struct.unpack_from('<I', data, 11)[0]
                if cseq != 0x0FFFFFFF and cseq >= getattr(ch, 'client_seq', 0):
                    ch.client_seq = cseq

            if not ch.established and msg_id == 0x01:
                ch.established = True
                self._log('канал установлен (on-channel SessionKey 0x01)', addr)
                self._send_ack(addr, ch)            # ПОДТВЕРДИТЬ SessionKey echo (reliable!)
                self._send_player_bootstrap(addr, ch)
                return

            # подтверждать reliable-пакет клиента его НАСТОЯЩИМ seq, иначе resend
            if (pflags2 := struct.unpack_from('<H', data, 4)[0]) & 0x0010:
    ...
    def _send_ack(self, addr, ch):
        """Шифрованный cumulative-ACK — подтверждаем reliable-пакеты клиента.
        Точная копия ack-only формата клиента (flags=0x1448, БЕЗ checksum):
          flags(2) + last_reliable(4) + seq(4) + cumulative_ack(4),
        затем prefix(4) + encrypt(всё после префикса). cumulative_ack =
        наибольший seq, полученный от клиента (ch.client_seq).
        Старая версия слала это build_bw_packet'ом БЕЗ ШИФРА -> клиент дропал
        (канал зашифрован) -> клиент вечно пере-слал reliable + пиггибэкал."""
        SENT_NULL = 0x0FFFFFFF   # sentinel last_reliable (как у клиента)
        flags = 0x0008 | 0x0040 | 0x0400 | 0x1000   # ON_CHANNEL|HAS_SEQ|CUM_ACK|0x1000 = 0x1448
        last_reliable = getattr(ch, 'last_reliable_sent', SENT_NULL)
        body  = struct.pack('<H', flags)
        body += struct.pack('<I', last_reliable & 0xFFFFFFFF)          # last_reliable
        body += struct.pack('<I', ch.our_seq)                       # FIX#20: PEEK seq (non-reliable ACK НЕ тратит reliable-номер!)
        body += struct.pack('<I', getattr(ch, 'client_seq', 0) & 0xFFFFFFFF)  # cumulative_ack
        offset = getattr(ch, 'prefix_offset', 0)
        prefix = _prefix_hash(bytes(body), offset)
        pkt = ch.encrypt_packet(struct.pack('<I', prefix) + bytes(body))  # prefix клиром, шифр
        self.sock.sendto(pkt, addr)
        self._log(f'>> cum-ACK seq client={getattr(ch,"client_seq",0)} {len(pkt)}B', addr)
